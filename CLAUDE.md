...
### Theme capture submodule

`lp-lineage-theme-capture/` is a git submodule (`https://github.com/sdaitzman/lp-lineage-theme-capture`) containing:
- **`captured-themes/`** — Crawled HTML, CSS, screenshots, and style guides for each LP sub-site
- **`plans/`** — Repeatable workflow documents (e.g. `subsite-theme-replication.md`)

This submodule keeps large reference assets and instructions out of the theme code repo. When committing changes to files inside the submodule, commit within the submodule first, then commit the updated submodule reference in the parent repo.
...
- **Goal:** Recreating specific sub-sites of `https://landscapepartnership.org` within this theme. Reference crawls and style guides are in `lp-lineage-theme-capture/captured-themes/` (a git submodule).
